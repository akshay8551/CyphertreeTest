...
class CreateTrigger02 extends React.Component {
    ...
    getGoogleSpreadSheet = () => {
        if (this.state.refreshToken == '') {
            ...
            var g_token = this.state.refreshToken
        }
    ...
    refreshToken = async () => {
        ...
        var user_token = userToken.replace(/"/g, '')
        ...
        fetch("http://demo-candi-server.herokuapp.com/api/v1/google/auth/refresh/", apiHeader
        ).then((response) => response.json())
            .then((responseJson) => {
                console.log('refresh', responseJson.data.google_access_token)
